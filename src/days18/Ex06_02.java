...
//			ts.add(score[i]);
//		} // for i
//		System.out.println(ts); // [10, 45, 50, 65, 80, 85, 95, 100] 트리로 자동정렬
		
		// int [] -> IntSteam
		Arrays.stream(score).forEach(ts::add);
		System.out.println(ts); // 위 for i 와 같은 코딩 [10, 45, 50, 65, 80, 85, 95, 100]
		...
		// TreeSet 중복허용 X, (자동 오름차순됨) 정렬, 검색, 부분검색 성능 뛰어나다.
		...
		// 정렬된 Set
		SortedSet<String> ss = ts.subSet("a", "d"); // "a"<= ~ <"d" 알파벳 오름차순으로 정렬
