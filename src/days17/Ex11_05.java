...
public class Ex11_05 {
	...
	public static void displayLottoNumbers( HashSet lottoSet ) {      
		...
		// HashSet lottoSet : 순서유지 X (오름차순 정렬도 안됨), 중복허용 X
		//		   LinkedHashSet 컬렉션 클래스 : 순서유지 O , 중복허용 X
		// ArrayList 컬렉션 클래스로 변환 : 순서유지 O, 중복허용 O (값은 이미 들어가있으니 상관없음)

		// HashSet -> ArrayList 변환 -> 오름차순 정렬 -> 출력
		//      List<Integer> lotto = new ArrayList<>(lottoSet);
		ArrayList lotto = new ArrayList<>(lottoSet);
		//      Collections.sort(lotto); // Collections 클래스 Arrays 클래스
