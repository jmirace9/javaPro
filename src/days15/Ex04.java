...
class Outer { // 외부 클래스
	...
	// 1. 인스턴스 클래스
	class InstanceClass{
		// 주로 외부클래스의 인스턴스 멤버들과 관련된 작업을 더 쉽게하기 위해서
		// 인스턴스 클래스로 선언한다.
	}
	
	// 2. 정적(static) 클래스
	static class StaticClass{
		// 주로 외부클래스의 static 멤버들과 관련된 작업을 더 쉽게하기 위해서
		// 인스턴스 클래스로 선언한다.
	}
	
	{ // 인스턴스 초기화 블럭
		class LocalClass{
			// 3. 지역(local) 클래스 - 인스턴스 초기화 블럭 안에서만 사용되는 클래스
		}
	...
	void disp() {
		...
		// 3. 지역(local) 클래스 - 외부 클래스의 메서드 안에서만 사용되는 클래스
		class localClass2{
